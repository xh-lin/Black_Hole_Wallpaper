...
class BlackHole{
  ...
  /* sphereRadius, diskColors, diskStart, diskRange, diskThickness, 
    diskParticleCount, diskParticleSize, diskTexture, 
    jetEnabled, jettingInterval,
    jetParticleCount, jetParticleSize, jetTexture,
    starDstStart, starDstRange,
    starParticleCount, starSize, starTexture, */
  constructor() {

    // black sphere (event horizon)
    this.sphereRadius = 90;
    const sphereGeo = new THREE.SphereGeometry( this.sphereRadius, this.sphereRadius, this.sphereRadius );
  ...
  onAnimate() {
    ...
    const c = 30; // overall speed
    const p = 1.7; // how much faster when closer to the center
    const d = 50; // shift the distance for speed calculation
